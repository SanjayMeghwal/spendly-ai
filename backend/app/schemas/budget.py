...
class BudgetCreate(BaseModel):
    ...
    category: str = Field(
        min_length=1,
        max_length=100,
        description="Free-text category this budget limits.",
        examples=["Groceries"],
    )
...
class BudgetUpdate(BaseModel):
    ...
    fields back NOT NULL columns, so - unlike TransactionUpdate's category
    ...
    # `str | None` exists only so the field can be OMITTED, but that same
    # type would also accept an explicit `null`, which would reach the
    # database as a NOT NULL violation - an unhandled 500 - rather than a
    # clean validation error.
    @field_validator("category", "limit_amount", mode="before")
    @classmethod
...
class BudgetRead(BaseModel):
    ...
    No `user_id` field, for the same reason TransactionRead has none. `spent`
    and `remaining` are NOT columns on the Budget model - they are computed
    per request from `transactions` by app/services/budget.py, for whichever
    month was asked for (the current UTC month, by default). Because they
    aren't ORM attributes, routes build this model explicitly rather than via
    `model_validate(budget)` alone - see app/api/routes/budgets.py.
    """
