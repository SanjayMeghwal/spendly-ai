...
| | |
|---|---|
| Milestone | **1 — "Hello, Production"** (walking skeleton) — complete |
| Done | git hygiene · uv deps · Postgres+pgvector container · validated config · async engine + session · FastAPI app · liveness/readiness probes · Alembic (baseline) · tests (91%) · CI |
| Next | **Milestone 2 — authentication and users**: first real model, first autogenerated migration, password hashing, JWT |
| Not started | transactions, budgets, frontend, AI (later milestones) |
