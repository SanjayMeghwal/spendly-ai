...
| Done (M2) | `User` model (UUIDv7 PK) · first autogenerated migration · argon2id hashing · JWT access tokens · `POST /auth/register` · `POST /auth/login` · `GET /users/me` · `get_current_user` · transactional test fixtures · 102 tests, 99% |
| Next | **Milestone 3** — the tech-stack note below says frontend; income/expense tracking (first `user_id` FK) is the other candidate. Not yet decided. |
| Not started | transactions, budgets, frontend, AI |
...
pydantic-settings · PyJWT · argon2-cffi · uv
...
| **UUIDv7 primary keys**, not bigserial | Sequential IDs in URLs let anyone enumerate accounts and read the signup rate. v7 is unguessable but time-ordered, so inserts still append to the index instead of fragmenting it. Needs `uuid-utils` (Python 3.12 has no `uuid7()`, PG17 has no `uuidv7()`) |
| **argon2id** via argon2-cffi, not bcrypt | OWASP's first recommendation; memory-hard, so GPU/ASIC cracking loses most of its advantage. No 72-byte silent truncation |
| **PyJWT**, not python-jose | jose is effectively unmaintained and has had CVEs |
| **No passlib** | Last release 2020; breaks against bcrypt ≥ 4.1. argon2-cffi is called directly |
| **Access token only** (30 min), refresh tokens deferred | Smallest correct slice. ⚠️ Consequence: there is no logout and no revocation — a stolen token works until it expires |
| **`Authorization: Bearer`**, not httpOnly cookie | Standard for APIs, keeps the /docs Authorize button working, no CSRF surface. ⚠️ Cost: the React app must store the token, and localStorage is readable by XSS — revisit when the frontend lands |
...
**Tests need their own engine (`NullPool`).** pytest-asyncio gives every test a
fresh event loop, and asyncpg binds each connection to the loop that opened it.
Reusing the application's pooled engine hands a test a connection belonging to a
dead loop — surfacing as "attached to a different loop", or a hang, in a test
that has nothing to do with the one that poisoned the pool. `conftest.py` builds
a per-test engine with `NullPool` so nothing outlives its loop.

**Committing inside a test (`join_transaction_mode="create_savepoint"`).** The
service layer really calls `commit()` — that is the behaviour under test. A real
COMMIT would end the fixture's outer transaction and make its rollback a no-op,
leaking rows between tests. That argument makes the session emit
SAVEPOINT/RELEASE instead of BEGIN/COMMIT, so commits behave correctly *and* the
enclosing transaction still rolls back.

**Config tests must pass `_env_file=None`.** Otherwise pydantic-settings loads
the developer's real `.env`, and a test asserting that a missing field is
rejected passes because `.env` supplied it. The test cannot fail — worse than
not having it. Delete the matching `os.environ` entry too, since CI sets one.
